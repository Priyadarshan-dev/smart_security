...
class _VehicleEntryScreenState extends ConsumerState<VehicleEntryScreen> {
  ...
  void initState() {
    ...
    vehicleHistoryScrollController.addListener(_vehicleHistoryScrollListener);
  }
  ...
  void _vehicleHistoryScrollListener() {
  ...
  void _showEditVehicleDialog(BuildContext context, dynamic vehicle) {
    ...
    // Pre-fill purpose controllers
    if (vehicle['purpose'] != null) {
      final details = vehicle['purpose'].toString();
      switch (details) {
        case "Employee":
    ...
    // Pre-fill type controllers
    if (vehicle['vehicleType'] != null) {
      final details = vehicle['vehicleType'].toString();
      switch (details) {
        case "CAR":
              ...
              return RefreshIndicator(
                ...
                child: ListView.builder(
                  ...
                  itemCount:
                      ...
                      (ref.watch(securityProvider).hasMoreVehicles ? 1 : 0),
                  itemBuilder: (context, index) {
  ...
  Widget _buildVisitTypeFields(
    ...
    // No comments fields needed in vehicle dialogs
    if (selectedPurpose == "Visitor") {
      return Column(
        children: [
          const SizedBox(height: 20),
          AddressProofWidget(
            image64: _addressProofImage64,
            onCapture: () => _pickAddressProofImage(setDialogState),
            onRemove: () {
              setDialogState(() {
  ...
  String _getVisitDetails(String? purpose) {
    switch (purpose) {
      case "Employee":
        return _employeeController.text;
      case "Visitor":
        return _visitorController.text;
      case "Delivery":
        return _deliveryController.text;
      case "Vendor":
        return _vendorController.text;
      case "Other":
